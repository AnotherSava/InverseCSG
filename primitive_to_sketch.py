...
def ComputeOrthonormalAxis(planes):
  ...
  for p in planes:
    new_norm = p.norm
    if IsNew(new_norm, all_norms, SameVector3d, 1e-3):
  ...
  for norm1 in all_norms:
    for norm2 in all_norms:
      ...
      if not SameVal(np.dot(norm1, norm2), 0, 1e-3): continue
      norm3 = np.cross(norm1, norm2)
      for norm in all_norms:
        if SameVector3d(norm3, norm, 1e-3) or SameVector3d(-norm3, norm, 1e-3):
          norm3 = norm
          break
      # Now (norm1, norm2, norm3) are orthonormal.
      new_coord = NormalizeCoordinates(Coordinates(norm1, norm2, \
        Vector3d(*norm3)))
      if IsNew(new_coord, coord_sets, SameCoordinates, 1e-3):
  ...
  for norm1 in left_over_norms:
    if abs(norm1.x) + abs(norm1.y) < 1e-3:
...
def GetSphereOffsetsAlongCoords(spheres, coord):
  ...
  for s in spheres:
    ...
    if IsNew(x, offsets_x, SameVal, 1e-3): offsets_x.append(x)
    if IsNew(y, offsets_y, SameVal, 1e-3): offsets_y.append(y)
    if IsNew(z, offsets_z, SameVal, 1e-3): offsets_z.append(z)
...
# Assume GetAllPrimitives have been called. This function detects primitives in
# a particular local region.
def GetAllPrimitivesInRoi(max_offset, input_primitive_file, \
  input_meta_point_file, output_primitive_file, output_sketch_file, roi):
  spheres, planes, cylinders, tori = DetectPrimitivesInRoi(input_primitive_file, \
    input_meta_point_file, output_primitive_file, roi)
  GenerateAuxiliaryPrimitivesAndFiles(spheres, planes, cylinders, tori, \
    max_offset, output_primitive_file, output_sketch_file)

# Given a mesh file, return all detected and auxiliary primitives. Write them
# into a file out_primitive_file. Max offset is a big number such that a sphere
# centered at the origin with radius = max_offset will enclose the mesh. A good
# choice would be slightly bigger than a half of the diagonal of the bounding
# box.
# output_initial_primitive_file will be the list of all primitives detected by
# the cpp command. output_primitive_file includes all of them plus auxiliary
# primitives, which are used by the sketch program.
def GetAllPrimitives(mesh_file, max_offset, output_initial_primitive_file, \
  output_meta_point_file, output_primitive_file, output_sketch_file, \
  options):
  spheres, planes, cylinders = DetectPrimitives(mesh_file, \
    output_initial_primitive_file, output_meta_point_file, options)
  GenerateAuxiliaryPrimitivesAndFiles(spheres, planes, cylinders, \
    max_offset, output_primitive_file, output_sketch_file)

def GenerateAuxiliaryPrimitivesAndFiles(spheres, planes, cylinders, tori, \
  max_offset, output_primitive_file, output_sketch_file):
  ...
  for coord in coords:
    ...
    for d in offsets_x:
      ...
      if IsNew(plane, all_planes, SamePlane, 1e-3):
  ...
  # Write planes, spheres, and cylinders into files.
  f = open(output_primitive_file, 'w')
  for p in all_planes:
  ...
  for t in tori:
    ...
    f.write('torus %f %f %f %f %f %f %f %f\n' % \
      (center.x, center.y, center.z, norm.x, norm.y, norm.z, \
       major_radius, minor_radius))
  f.close()

  # Generate Sketch file.
  WriteSketchFile(output_sketch_file, spheres, rotations_and_offsets, \
    cylinder_hints, tori)

def WriteSketchFile(file_name, spheres, rotations_and_offsets, cylinder_hints, \
  tori):
